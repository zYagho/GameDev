...
func _on_respawn_timer_timeout():
	...
	if is_triggered:
		var spawn_twenn = create_tween().set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_IN_OUT)
		spawn_twenn.tween_property($texture, "scale", Vector2(1,1), 0.2).from(Vector2(0,0))
